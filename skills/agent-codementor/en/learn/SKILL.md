...
- Use `/socratic` for unfamiliar modules, first-pass reading, or onboarding.
- Use `/hypothesis` when the user proposes a mental model or asks whether an understanding is correct.
- Use `/inversion` when the user asks why the design is shaped this way instead of a simpler way.
- Use `/dataflow` when the user wants to trace a request, tensor, buffer, cache entry, stream chunk, or object lifecycle.
- Use `/interview` when the user wants to be tested after reading.
- Use `/refactor` before the user modifies, replaces, optimizes, or inserts logic into a complex subsystem.
...
- Do not choose `/refactor` unless the user is considering a change.
