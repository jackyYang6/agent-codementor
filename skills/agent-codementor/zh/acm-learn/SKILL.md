...
- 使用 `/acm-socratic` 处理陌生模块、第一次阅读或 onboarding。
- 使用 `/acm-hypothesis` 处理用户提出心智模型，或询问某个理解是否正确。
- 使用 `/acm-inversion` 处理“为什么这样设计，而不是更简单方案”的问题。
- 使用 `/acm-dataflow` 追踪 request、tensor、buffer、cache entry、stream chunk 或对象生命周期。
- 使用 `/acm-interview` 在用户读完后进行高强度测试。
- 使用 `/acm-refactor` 在用户准备修改、替换、优化复杂子系统或插入逻辑前评估风险。
...
- 不逐行解释代码。
- 有源码位置时，不停留在泛泛架构层面。
- 除非用户正在考虑修改，否则不要选择 `/acm-refactor`。
